	...
	const FileBrowser &copyFrom)
{
	...
	*inputNameBuf_ = *copyFrom.inputNameBuf_;
	...
	openNewFileLabel_ = copyFrom.openNewFileLabel_;
	newFileNameBuf_ = std::make_unique<
		std::array<char, INPUT_NAME_BUF_SIZE>>();
	*newFileNameBuf_ = *copyFrom.newFileNameBuf_;
...
void ImGui::FileBrowser::SetTitle(std::string title)
{
	...
	openNewFileLabel_ = "new file##new_file_" +
		std::to_string(reinterpret_cast<size_t>(this));
}
...
void ImGui::FileBrowser::Display()
{
	...
	if (Button("New"))
	{
		OpenPopup(openNewFileLabel_.c_str());
		(*newFileNameBuf_)[0] = '\0';
	}

	if (BeginPopup(openNewFileLabel_.c_str()))
	{
		ScopeGuard endNewFilePopup([] { EndPopup(); });

		bool rez = InputText("name", newFileNameBuf_->data(), newFileNameBuf_->size(), ImGuiInputTextFlags_EnterReturnsTrue);
		SameLine();

		if ((Button("ok") || rez) && (*newFileNameBuf_)[0] != '\0')
		{
			ScopeGuard closeNewFilePopup([] { CloseCurrentPopup(); });
			std::ofstream f(pwd_ / newFileNameBuf_->data());
			if (f.is_open())
			{
				f.close();
				UpdateFileRecords();
			...
			{
				statusStr_ = "failed to create " +
					std::string(newFileNameBuf_->data());
			}
		}
	}
}
